...
public sealed class RoomHandleManager : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        bool visible = modeManager == null ||
                       modeManager.IsMode(EditorMode.RoomCreate);
    ...
    private void UpdateDraggingGroup()
    {
        ...
        if (!HasValidArea(updatedVertices) || !HasMinimumEdgeLength(updatedVertices) || !IsSimplePolygon(updatedVertices))
        {
            return;
        }
    ...
    private void EndDrag()
    {
        ...
        if (draggingGroup.room != null && dragOriginalVertices.Count >= 3)
        {
            ...
            if (finalVertices.Count < 3 || !HasValidArea(finalVertices) || !HasMinimumEdgeLength(finalVertices) || !IsSimplePolygon(finalVertices))
            {
    ...
    private bool HasMinimumEdgeLength(List<Vector3> vertices)
    {
        for (int i = 0; i < vertices.Count; i++)
        {
            Vector3 current = vertices[i];
            Vector3 next = vertices[(i + 1) % vertices.Count];
            if ((next - current).sqrMagnitude < minimumRoomEdgeLength * minimumRoomEdgeLength)
            {
                return false;
            }
        }
    ...
    private static bool HasValidArea(List<Vector3> vertices)
    {
        return Mathf.Abs(CalculateSignedArea(vertices)) > PolygonAreaEpsilon;
    }

    private static bool IsSimplePolygon(List<Vector3> vertices)
    {
    ...
    private static float CalculateSignedArea(List<Vector3> vertices)
    {
        float area = 0f;
        for (int i = 0; i < vertices.Count; i++)
        {
